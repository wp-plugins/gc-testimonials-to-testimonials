...
if ( ! function_exists( 'af_php_version_check' ) ) {
	...
	function af_notice_php_version() {
		...
		$text = sprintf( __( 'Plugin "%1$s" requires at least PHP %2$s, you\'re running PHP %4$s. See <a href="%3$s">possible solutions</a>. Once fixed, "%1$s" can be activated again..', 'gc-testimonials-to-testimonials' ), $base, $php_min, $help_url, PHP_VERSION );
...
EOD;
	...
	public static function notice_version( $free_base, $free_name, $free_slug, $free_version, $item_name ) {
		...
		$text = sprintf( __( 'Plugin %3$s has been deactivated. Please %1$s %4$s version %2$s or newer before activating %3$s.', 'gc-testimonials-to-testimonials' ), $link, $free_version, $item_name, $free_name );
	...
	public static function notice_license( $post_type, $settings_id, $free_name, $purchase_url, $item_name ) {
		...
		$text = sprintf( __( 'Plugin %1$s requires license activation before updating will work. Please activate the license key via %2$s. No license key? See %3$s or purchase %4$s.', 'gc-testimonials-to-testimonials' ), $item_name, $settings_link, $faq_link, $buy_link );
	...
	public static function notice_donate( $disable_donate = null, $item_name = null ) {
		...
		$text = sprintf( esc_html__( 'Please donate $5 towards ongoing free support and development of the %1$s plugin. %2$s', 'gc-testimonials-to-testimonials' ), $item_name, self::$donate_button );
